...
export default function Warehouse1Report() {
  ...
  const allRecords = useLiveQuery(() => 
    db.records.where('section').anyOf(['warehouse_1', 'wh1_receiving']).toArray()
  );
  ...
  const summary = {
    ...
    receivedCount: filteredRecords.filter(record => record.section === 'wh1_receiving').length,
    issuedCount: filteredRecords.filter(record => record.section === 'warehouse_1').length,
    totalReceivedQty: filteredRecords
      .filter(record => record.section === 'wh1_receiving')
      .reduce((sum, record) => sum + (Number(record.data.consumedQuantity) || 0), 0),
    totalIssuedQty: filteredRecords
      .filter(record => record.section === 'warehouse_1')
      .reduce((sum, record) => sum + (Number(record.data.consumedQuantity) || 0), 0),
  };
  ...
  const generatePDF = () => {
    ...
    const summaryData = {
      ...
      [t('total_quantity_received', 'Total Received')]: summary.totalReceivedQty,
    ...
    const tableColumn = [
      ...
      t('batch_lot', 'Batch/Lot'), 
      t('receiver_name', 'Receiver'),
      t('type', 'Type')
    ];
    
    const tableRows = records.map(record => {
      let type = record.section === 'wh1_receiving' ? t('received', 'Received') : t('issued', 'Issued');
      return [
    ...
    const filename = `WH1_Report_${reportType}_${format(new Date(), 'yyyyMMdd_HHmm')}.pdf`;
    generateHtmlPdf(t('report_title_warehouse_1_receiving_report', 'Warehouse 1 Report'), reportPeriod, summaryData, tableColumn, tableRows, filename, isRTL, t);
  };
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <h1 className="text-2xl font-bold tracking-tight">WH1 Reports</h1>
      </div>

      <Card>
        <CardHeader>
          <CardTitle className="flex items-center gap-2">
            <FileDown className="w-5 h-5 text-primary" />{t('generate_pdf_report', 'Generate PDF Report')}</CardTitle>
          <CardDescription>
            Export transactions, balances, and movement history for Raw Materials Warehouse 1.
          </CardDescription>
        </CardHeader>
        <CardContent className="space-y-6">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            ...
            <Card className="bg-emerald-500/5 border-emerald-500/20 shadow-sm">
              <CardContent className="p-4 flex flex-col items-center justify-center text-center h-full">
                <span className="text-sm text-muted-foreground mb-1">Items Received</span>
            ...
            <Card className="bg-amber-500/5 border-amber-500/20 shadow-sm">
              <CardContent className="p-4 flex flex-col items-center justify-center text-center h-full">
                <span className="text-sm text-muted-foreground mb-1">Items Issued</span>
                <span className="text-2xl font-bold text-amber-600">{summary.issuedCount}</span>
              </CardContent>
            </Card>
            <Button 
              className="h-full w-full py-8 text-base shadow-md"
              onClick={generatePDF}
                      ...
                      <tr key={record.id} className="hover:bg-muted/30">
                        ...
                        <td className="px-4 py-2">
                          <span className={`px-2 py-0.5 rounded-full text-xs font-medium ${
                            record.section === 'wh1_receiving' 
                              ? 'bg-emerald-100 text-emerald-700' 
                              : 'bg-amber-100 text-amber-700'
                          }`}>
                            {record.section === 'wh1_receiving' ? 'IN' : 'OUT'}
                          </span>
                        </td>
                        <td className="px-4 py-2">{record.data.materialType ? record.data.materialType.replace('mat_', '') : '-'}</td>
                        <td className="px-4 py-2 text-right font-medium">{record.data.consumedQuantity || 0}</td>
                      </tr>
                    ))
                  ) : (
                    <tr>
