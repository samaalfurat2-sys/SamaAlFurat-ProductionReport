...
export default function AccountingReport() {
  ...
  const getFilteredRecords = () => {
    ...
    return allRecords.filter(record => {
      ...
      if (statusFilter !== "all" && record.status !== statusFilter) {
        // Special handling for accepted/pending logic across sections can be added here if needed
        return false;
      }
  ...
  const summary = {
    ...
    pendingCount: filteredRecords.filter(r => r.status?.includes('pending')).length,
    acceptedCount: filteredRecords.filter(r => r.status === 'accepted' || r.status === 'audited').length,
    rejectedCount: filteredRecords.filter(r => r.status?.includes('rejected')).length,
  };
  ...
  const generatePDF = () => {
    ...
    const summaryData = {
      ...
      [t('pending_wh3', 'Pending')]: summary.pendingCount,
      [t('accepted', 'Accepted')]: summary.acceptedCount,
    ...
    const filename = `Accounting_Report_${reportType}_${format(new Date(), 'yyyyMMdd_HHmm')}.pdf`;
    generateHtmlPdf(t('report_title_accounting_financial_report', 'Accounting Complete Transaction Report'), reportPeriod, summaryData, tableColumn, tableRows, filename, isRTL, t);
  };
    ...
    <div className="space-y-6">
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Export complete transaction history across all warehouses and production sections.
          </CardDescription>
        </CardHeader>
        <CardContent className="space-y-6">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            ...
            <Card className="bg-emerald-500/5 border-emerald-500/20 shadow-sm">
              <CardContent className="p-4 flex flex-col items-center justify-center text-center h-full">
                <span className="text-sm text-muted-foreground mb-1">Accepted</span>
                <span className="text-2xl font-bold text-emerald-600">{summary.acceptedCount}</span>
              </CardContent>
            </Card>
            <Button 
              className="h-full w-full py-8 text-base shadow-md bg-blue-600 hover:bg-blue-700 text-white"
              onClick={generatePDF}
                        ...
                        <tr key={record.id} className="hover:bg-muted/30">
                          ...
                          <td className="px-4 py-2">
                            <span className={`px-2 py-0.5 rounded-full text-xs font-medium ${
                              record.status?.includes('accepted') || record.status?.includes('audited')
